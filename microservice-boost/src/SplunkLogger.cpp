...
std::string SplunkLogger::LogEntry::toJson() const
{
...
std::string SplunkLogger::formatEntry(const std::string& entryJson) const
{
    ...
    oss << "{"
        << "\"event\":" << entryJson << ","
        << "\"index\":\"" << settings_->getIndex() << "\","
        << "\"sourcetype\":\"" << settings_->getSourceType() << "\""
        << "}";
    return oss.str();
}

void SplunkLogger::log(LogLevel level,
...
void SplunkLogger::scheduleFlush()
{
...
void SplunkLogger::onTimer(const boost::system::error_code& ec)
{
...
void SplunkLogger::sendBatch(const std::vector<LogEntry>& entries)
{
    std::string url = settings_->getUrl();
    if (url.empty()) {
        fallbackLogger_->log(LogLevel::Warn, "SplunkLogger", "URL not configured");
        return;
    }
    ...
    std::string token = settings_->getToken();
    if (!token.empty()) {
        request->headers_["Authorization"] = "Splunk " + token;
    }
    ...
    if (!result.ok()) {
        fallbackLogger_->log(LogLevel::Error, "SplunkLogger",
            std::string("Failed to send logs: ") + result.errorMessage);
    }
}

void SplunkLogger::flush()
{
    doFlush();
}

void SplunkLogger::stop()
{
    stopped_ = true;
    timer_.cancel();
...
void SplunkLogger::shutdown(std::chrono::milliseconds timeoutMs)
{
    stopped_ = true;
    timer_.cancel();
    ...
    if (!entries.empty()) {
        ...
        if (sendRemaining > 0) {
            ...
            fallbackLogger_->log(LogLevel::Warn, "SplunkLogger",
